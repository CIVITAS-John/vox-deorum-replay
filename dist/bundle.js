(function () {
    ...
    function checkPlotHead(body, s) {
        if (s < 0 || s + PLOT_YIELDS_OFFSET + PLOT_TEAM_SEARCH + 400 >= body.length)
            return null;
    ...
    function checkTeamBlock(body, b) {
        ...
        for (let q = b + PLOT_YIELDS_OFFSET; q < b + PLOT_YIELDS_OFFSET + PLOT_TEAM_SEARCH; q++) {
            if (isRunStart(q) >= 8 && runQualifies(q))
                return true;
        }
        return false;
    }
     ...
     */
    function countRecordsBetween(body, from, to) {
        const runs = [];
     ...
     */
    function findNextPlotRecord(body, s, minDist) {
    ...
    function extractMapTerrain(body, mapPos, width, height) {
        ...
        const validateStart = (pos) => {
            if (checkPlotHead(body, pos) === null)
                return false;
            if (!checkTeamBlock(body, checkPlotHead(body, pos).riversBase))
                return false;
