(function () {
     ...
     */
    function checkPlotHead(body, s) {
        if (s < 0 || s + PLOT_BLOCK_OFFSET + PLOT_TEAM_BLOCK_SIZE + 400 >= body.length)
            return null;
        const i16 = (p) => ((body[p] | (body[p + 1] << 8)) << 16) >> 16;
        const i8 = (p) => (body[p] << 24) >> 24;
        const u8 = (p) => body[p];
        const i32 = (p) => (body[p] | (body[p + 1] << 8) | (body[p + 2] << 16) | (body[p + 3] << 24));
        const u32 = (p) => ((body[p] | (body[p + 1] << 8) | (body[p + 2] << 16) | (body[p + 3] << 24)) >>> 0);
        if (i16(s) < -1 || i16(s) > 8192)
            return null;
        if (i16(s + 2) < -1 || i16(s + 2) > 20000)
            return null;
        if (i16(s + 4) < -1 || i16(s + 4) > 20000)
            return null;
        if (i16(s + 6) < -1 || i16(s + 6) > 20000)
            return null;
        for (let k = 0; k < 5; k++) {
            const c = i8(s + 8 + k);
            if (c < -1 || c > 63)
        ...
        if (i16(s + 13) < -1 || i16(s + 13) > 4096)
            return null;
        if (i16(s + 15) < -1 || i16(s + 15) > 4096)
            return null;
        ...
        if (u16le(body, b + PLOT_BITS_OFFSET) > 0x7FFF)
            return null;
        for (let k = 0; k < 5; k++) {
            const e = i8(b + 23 + k);
            if (e < -1 || e > 127)
     ...
     */
    function checkTeamBlock(body, b) {
        ...
        for (let k = 0; k <= maxEntry; k++) {
            ...
            if (reps >= PLOT_TEAM_RUN_MIN && runQualifies(body, q)) {
                ...
                if (runEnd >= blockEnd - 96 && runEnd <= blockEnd)
                    return true;
            }
        }
        return false;
     ...
     */
    function isTrustedTerrain(head) {
     ...
     */
    function findNextPlotRecord(body, blockEnd) {
        ...
        for (let t = from; t + 21 < limit; t++) {
            ...
            if (head && checkTeamBlock(body, head.riversBase))
                return cand;
        }
        return -1;
    }
     ...
     */
    function skipVectorBody(body, view, p, max, elementSize) {
     ...
     */
    function parseTailToUnits(body, view, blockEnd) {
     ...
     */
    function countSkippedRecords(body, from, to) {
        ...
        for (const [start, len] of runs) {
            if (lastEnd < 0 || start - lastEnd > 128)
                records++;
        ...
        for (let p = Math.max(0, from); p < to; p++) {
            if (body[p] === 0) {
                ...
                if (run >= PLOT_BLANK_MIN_SIZE)
                    records += Math.floor(run / PLOT_BLANK_RECORD_SIZE);
                run = 0;
            }
        }
        if (run >= PLOT_BLANK_MIN_SIZE)
     ...
     */
    function walkPlotSegment(body, start, maxSlots) {
        ...
        while (tiles.length < maxSlots) {
            ...
            if (unitsEnd >= 0) {
                ...
                for (let cand = unitsEnd + 16; cand <= to; cand++) {
                    ...
                    if (candHead && checkTeamBlock(body, candHead.riversBase)) {
                        next = cand;
                        break;
                    }
                    if (isBlankRecord(body, cand)) {
                        next = cand;
                        break;
                    }
                }
    ...
    function extractMapTerrain(body, mapPos, width, height) {
        ...
        const validateStart = (pos) => {
            const head = checkPlotHead(body, pos);
            if (!head || !isTrustedTerrain(head) || !checkTeamBlock(body, head.riversBase))
                return false;
